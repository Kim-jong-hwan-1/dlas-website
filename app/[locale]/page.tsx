...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const dismissed = localStorage.getItem("DLAS_LIFETIME_DISMISSED");
    if (!dismissed || Date.now() - Number(dismissed) > 24 * 60 * 60 * 1000) {
      setShowLifetimeModal(true);
    }
  }, [introFinished]);

  const dismissLifetimeForDay = () => {
    localStorage.setItem("DLAS_LIFETIME_DISMISSED", String(Date.now()));
  ...
  return (
    <>
      ...
      {showLifetimeModal && (
        ...
        >
          <div
            className="relative bg-black/20 backdrop-blur-xl border border-white/10 rounded-2xl p-4 sm:p-6 max-w-2xl w-full"
          ...
          >
            ...
            <div className="relative w-full aspect-square max-h-[75vh] rounded-xl overflow-hidden">
              <Image
                src="/notice/lifetime-discontinued.png"
                alt="평생 라이센스 판매 중단 공지"
                fill
