...
// 모듈별 가격 오버라이드 (KRW)
const MODULE_PRICE_OVERRIDE_KRW: Record<string, Record<string, number>> = {
  "stl_classifier": { "1YEAR": 2_200_000, "LIFETIME": 5_500_000 },
};
...
const priceLabelForModule = (
  ...
  // 모듈별 가격 오버라이드 확인
  const override = MODULE_PRICE_OVERRIDE_KRW[mod]?.[period];
  if (override && isKrwDisplay(country)) {
    return `₩${override.toLocaleString()}`;
  }
...
export default function BuyPage() {
  ...
  const [activeTab, setActiveTab] = useState<'automation' | 'fastEditor' | 'sorter' | null>('automation');
  ...
  const [pendingPayment, setPendingPayment] = useState<{
    type: "module" | "permanent" | "family" | "family50" | "fastEditor" | "sorter";
    module?: string;
    period?: string;
  ...
  // Automatic Sorter 결제 핸들러 - 사업자 변경으로 일시 중단
  const handleSorterPayment = (period: string) => {
    setShowPaymentBlockedModal(true);
    return;
  };
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col w-full sm:hidden items-center">
          ...
          {/* 트랜스퍼 지그 / STL Classifier는 1년/평생만 표시 */}
          {(mod === "3_transfer_jig_maker" || mod === "e_transfer_jig_maker" || mod === "stl_classifier") ? (
            <div className="flex flex-col w-full items-center gap-2">
              <div className="flex flex-row w-full justify-center items-center gap-2">
                <button
                  className="bg-black/30 border border-white/10 text-white rounded-lg w-1/3 h-14 text-base font-semibold flex flex-col items-center justify-center transition-all duration-300 hover:bg-black/50 hover:border-white/20"
                  onClick={() => setShowTransferJigContactModal(true)}
                >
                  <span className="text-lg leading-5">{t("buyPage.free3day")}</span>
                  <span className="text-sm leading-5 text-[#fde68a]">{t("buyPage.free")}</span>
                </button>
                <button
                  className="bg-black/30 border border-white/10 text-white rounded-lg w-1/3 h-14 text-base font-semibold flex flex-col items-center justify-center transition-all duration-300 hover:bg-black/50 hover:border-white/20"
                  onClick={() => setShowTransferJigContactModal(true)}
        ...
        <div className="hidden sm:flex flex-row items-center w-full h-full gap-6">
          ...
          {/* 트랜스퍼 지그 / STL Classifier: 3일무료 + 1년/평생 */}
          {(mod === "3_transfer_jig_maker" || mod === "e_transfer_jig_maker" || mod === "stl_classifier") ? (
            <div className="flex flex-col gap-3 w-40 flex-shrink-0 h-full justify-center items-center">
              <button
                ...
                onClick={() => setShowTransferJigContactModal(true)}
              >
                <span className="text-xl leading-5">{t("buyPage.free3day")}</span>
                <span className="text-base leading-5 text-[#fde68a]">{t("buyPage.free")}</span>
              </button>
              <button
                className="bg-black/30 border border-white/10 text-white rounded-lg w-36 h-16 text-lg font-semibold flex flex-col items-center justify-center transition-all duration-300 hover:bg-black/50 hover:border-white/20"
                onClick={() => setShowTransferJigContactModal(true)}
          ...
          >
            ...
            <div className="flex flex-col gap-y-16 w-full">
              ...
              {isKorean ? (
                      ...
                      <div className="flex flex-col items-center text-center">
                        ...
                        <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                          ...
                          >
                            ...
                            <span className="text-sm text-[#fde68a]">₩2,200,000</span>
                          </button>
                          <button
                          ...
                          >
                            ...
                            <span className="text-sm text-[#fde68a]">₩5,500,000</span>
                          </button>
                        </div>
                      </div>
                    </div>
                  ...
                  <>
                    ...
                    >
                      <div className="flex flex-col items-center text-center">
                        ...
                        <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                          <button
                            onClick={() => handleSorterPayment("1YEAR")}
                            className="bg-black/30 border border-white/10 text-white rounded-lg px-4 py-4 font-semibold flex flex-col items-center justify-center transition-all duration-300 hover:bg-black/50 hover:border-white/20"
                          >
                            <span className="text-lg">{t("buyPage.year1") || "1년"}</span>
                            <span className="text-sm text-[#fde68a]">₩2,200,000</span>
                          </button>
                          <button
                            onClick={() => handleSorterPayment("LIFETIME")}
                            className="bg-black/30 border border-white/10 text-white rounded-lg px-4 py-4 font-semibold flex flex-col items-center justify-center transition-all duration-300 hover:bg-black/50 hover:border-white/20"
                          >
                            <span className="text-lg">{t("buyPage.lifetime") || "평생"}</span>
                            <span className="text-sm text-[#fde68a]">₩5,500,000</span>
                          </button>
                        </div>
                      </div>
                ...
                /* 해외: 한국과 동일하게 두 버튼 표시, 자동화 모듈 클릭 시 안내 모달 */
                activeTab === null ? (
                  /* 초기 화면: 두 버튼 표시 (중앙 정렬) */
                  <div className="flex flex-col sm:flex-row justify-center gap-6 sm:gap-12 w-full px-4">
                    <button
                    ...
                    >
                      <div className="flex flex-col items-center text-center">
                        ...
                        <div className="grid grid-cols-2 gap-4 w-full max-w-md">
                          ...
                          >
                            <span className="text-lg">{t("buyPage.year1")}</span>
                            <span className="text-sm text-[#fde68a]">$2,200</span>
                          </button>
                          <button
                            onClick={() => handleFastEditorPayment("LIFETIME")}
                            className="bg-black/30 border border-white/10 text-white rounded-lg px-4 py-4 font-semibold flex flex-col items-center justify-center transition-all duration-300 hover:bg-black/50 hover:border-white/20"
                          >
                            <span className="text-lg">{t("buyPage.lifetime")}</span>
                            <span className="text-sm text-[#fde68a]">$5,500</span>
                          </button>
                        </div>
                      </div>
                    </div>
            ...
            <div className="bg-white/5 border border-white/10 rounded-xl p-4 mb-6">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center justify-center gap-2 text-white/80">
                  ...
                  <span className="text-sm font-medium">032-212-2882</span>
                </div>
              </div>
            </div>
