...
def parse_construction_info(file_path, screw_teeth_set=None, save_excel=False):
    ...
    #    screw_teeth_set: { toothNum: groupIDs }를 만들어야 하는데,
    #    parse_construction_info 자체는 tooth->groupID를 알기 위해 final_df 이용
            ...
            for tooth_id, transformed_margin, locked_indices in margin_matches:
                if is_margin_fully_matched(transformed_margin, transformed_vertices, threshold=0.5):
                    ...
                    if len(transformed_margin) >= 3:
                        ...
                        plotter += Text3D(tooth_id, pos=label_pos + [0,0,1], s=1, c='green')
                    else:
...
def process_inner_hole(
    ...
    # 여기서 스크류채널도 그룹에 포함되도록(= parse_construction_info) 이용
    # parse 시, tooth2gids 이용하려면 치식 -> ???, 그러나 현재 parse 함수에서
    # 치식->그룹매핑은 final 단계에서 하므로 인자 없이 호출해도 됩니다.
    try:
        # parse_construction_info에선 "스크류채널" 행을 자동 추가
...
def run_gui():
    ...
    btn_infolder = ttk.Button(detail_frame, text="Select", command=select_input_folder, width=10)
    ...
    btn_add = ttk.Button(detail_frame, text="Add", command=add_files, width=10)
    btn_add.grid(row=2, column=2, padx=10, pady=(10,5), sticky="w")

    btn_del = ttk.Button(detail_frame, text="Remove", command=remove_selected_file, width=10)
    btn_del.grid(row=3, column=2, padx=10, pady=(5,5), sticky="w")

    btn_clear = ttk.Button(detail_frame, text="Clear", command=clear_files, width=10)
    ...
    global lbl_hole, hole_dia_entry
