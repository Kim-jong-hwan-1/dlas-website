...
from PySide6.QtWidgets import (
    ...
    QLabel, QMainWindow, QFrame, QCheckBox, QSpacerItem, QSizePolicy
)
...
class LoginWindow(QMainWindow):
    def __init__(self):
        ...
        logo_label = QLabel()
        logo_image_path = resource_path("white logo.jpg")
    ...
    def on_login_success(self, result):
        ...
        try:
            ...
            req = urllib.request.Request("http://localhost:8000/admin/my-license", headers=headers)
        ...
        try:
            for module in modules:
                ...
                subprocess.Popen([sys.executable, f"--module={normalized}"])
        except Exception as e:
            print("\u274c Module launch failed:", e)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1].startswith("--module="):
        mod = sys.argv[1].split("=")[-1]
        if mod == "fast_transfer_jig_maker":
            from modules import fast_transfer_jig_maker
            fast_transfer_jig_maker.run()
    else:
        app = QApplication(sys.argv)
        video_path = resource_path("intro.mp4")
