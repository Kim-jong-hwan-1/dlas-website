...
interface WebinaModalProps {
  onClose?: () => void;
}

export default function WebinaModal({ onClose }: WebinaModalProps) {
  ...
  // 웨비나 이미지 배열 (1.jpg ~ 3.jpg)
  const posters = Array.from({ length: 3 }, (_, i) => `/webina/${i + 1}.jpg`);
  ...
  const handleApply = () => {
    window.open(webinaLink, "_blank", "noopener,noreferrer");
  };
  ...
  return (
    ...
    >
      ...
      {/* 모달 헤더 */}
      <div className="bg-gradient-to-r from-purple-600 to-purple-800 text-white px-4 sm:px-6 py-3 sm:py-4">
        <h2 className="text-lg sm:text-xl lg:text-2xl font-bold text-center">웨비나 안내</h2>
        <p className="text-center mt-1 text-xs sm:text-sm text-purple-100">
          DLAS 웨비나에 참여하세요
        </p>
      </div>
      ...
      {/* 가격 정보 */}
      <div className="px-3 sm:px-4 py-2 bg-gradient-to-r from-purple-50 to-pink-50 border-t border-purple-100">
        ...
        <div className="flex justify-center items-center">
          <div className="text-center px-2 py-1 bg-white rounded shadow-sm">
            <span className="text-sm font-bold text-gray-800">7만원</span>
          </div>
        </div>
      ...
      <div className="px-3 sm:px-4 py-2 sm:py-3 bg-white flex gap-2">
        <button
          onClick={handleApply}
          className="px-4 sm:px-6 py-2 sm:py-3 rounded-lg bg-purple-600 text-white
